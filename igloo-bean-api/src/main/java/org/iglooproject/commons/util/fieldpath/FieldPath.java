...
/**
 * A symbolic representation of the "field path", i.e. the chain of properties that need to be
 * accessed in order to get a certain value from an object.
 *
 ...
 *   <li>".myProperty.subProperty": the path from the root to the "subProperty" property of
 *       "myProperty"
 *   <li>".myCollectionProperty[*]": the path from the root to an element of the (collection)
 *       property "myCollectionProperty"
 *   <li>".myCollectionProperty[*].subProperty": the path from the root to the "subProperty"
 *       property of an element of the (collection) property "myCollectionProperty"
 * </ul>
 *
 * <p>Note that these are <em>symbolic</em> representations, and as such they are not suitable for
 * automatic retrieval of values from a given root. For this sort of treatments, prefer the use of
 * {@link Binding Bindings}
 */
@Bindable
public class FieldPath
    implements Iterable<FieldPathComponent>,
        Serializable { // NOSONAR: this class CANNOT be final because of the ROOT static field.
  ...
  private static final StringMatcher DELIMITER_MATCHER =
  ...
  private FieldPath(
      Iterable<FieldPathComponent> components, Iterable<FieldPathComponent> addedComponents) {
    super();
    this.components =
        ImmutableList.<FieldPathComponent>builder()
            .addAll(components)
  ...
  private FieldPath(Iterable<FieldPathComponent> components, FieldPathComponent addedComponent) {
    super();
    this.components =
        ImmutableList.<FieldPathComponent>builder().addAll(components).add(addedComponent).build();
  }
  ...
  public Optional<FieldPath> relativeTo(FieldPath other) {
    if (startsWith(other)) {
      return Optional.of(
          new FieldPath(components.subList(other.components.size(), components.size())));
    } else {
      return Optional.absent();
    }
  }
