...
/**
 * A fixed DomainValidator that allows <em>any</em> TLD, and not only some hard-coded list, since
 * allowed TLDs may vary over time.
 *
...
public final class AnyTldDomainValidator extends RegexValidator implements Serializable {
  ...
  static final String DOMAIN_LABEL_REGEX =
      "[\\p{L}\\p{Digit}](?>[\\p{L}\\p{Digit}_-]*[\\p{L}\\p{Digit}])*";
