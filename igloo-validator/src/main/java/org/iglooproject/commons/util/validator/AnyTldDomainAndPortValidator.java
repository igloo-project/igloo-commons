...
/**
 * A fixed "authority" validator that allows <em>any</em> TLD, and not only some hard-coded list,
 * since allowed TLDs may vary over time.
 *
...
public final class AnyTldDomainAndPortValidator extends RegexValidator implements Serializable {
  ...
  private static final String DOMAIN_NAME_AND_PORT_REGEX =
      "^(?:"
          + AnyTldDomainValidator.DOMAIN_LABEL_REGEX
          + "\\.)+"
          + "("
          + AnyTldDomainValidator.TOP_LABEL_REGEX
          + ")"
          + "(\\:"
          + PORT_REGEX
          + ")?"
