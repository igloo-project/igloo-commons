...
/**
 * <b>Date</b> is 31/01/2018 14:32:02; we check that start and end of period, and previous, and next
 * are corrects. Period can be calendar month or calendar week (start on monday).
 *
 * <p>Built with <em>cal -3</em> command.
 *
 * <pre>
...
public class DateDiscreteDomainTest {
  ...
  public void dateWeekRange() {
    this.<LocalDateTime>range(
        DateDiscreteDomain.weeks(),
        LocalDateTime::parse,
        DATE,
        WEEK_PREVIOUS,
        WEEK_START,
  ...
  public void zonedDateTimeWeekRange() {
    this.<ZonedDateTime>range(
        ZonedDateDiscreteDomain.weeks(),
        ZonedDateTime::parse,
        DATE,
        WEEK_PREVIOUS,
        WEEK_START,
  ...
  public void zonedDateTimeMonthRange() {
    this.<ZonedDateTime>range(
        ZonedDateDiscreteDomain.months(),
        ZonedDateTime::parse,
        DATE,
        MONTH_PREVIOUS,
        MONTH_START,
  ...
  public <T extends Temporal & Comparable<? super T>> void range(
      PartitionDiscreteDomain<T> domain,
      BiFunction<String, DateTimeFormatter, T> generator,
      String value,
      String previous,
      String start,
      String end,
  ...
  public void serialize() throws IOException, ClassNotFoundException {
    ...
    DateDiscreteDomain domain =
        (DateDiscreteDomain)
            new ObjectInputStream(new ByteArrayInputStream(baos.toByteArray())).readObject();
    this.<LocalDateTime>range(
        domain, LocalDateTime::parse, DATE, WEEK_PREVIOUS, WEEK_START, WEEK_END, WEEK_NEXT);
  }
}
