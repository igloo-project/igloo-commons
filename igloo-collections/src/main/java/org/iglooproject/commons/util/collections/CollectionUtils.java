...
public final class CollectionUtils {
   ...
   *
   * <p>This method is view-proof : it will work as expected even if <code>src</code> is a view of
   * <code>dst</code>, i.e. if <code>src</code>'s content depends on <code>dst</code>'s (see for
   * example {@link Collections#unmodifiableCollection(Collection)}, {@link
   * Sets#union(java.util.Set, java.util.Set)}, ...).
   *
   ...
   * @param src The collection whose content is to be copied to <code>dst</code>, or null to simply
   *     clear <code>dst</code>.
   */
  ...
  public static <T> void replaceAll(
    ...
    src.stream().filter(((Predicate<T>) dst::contains).negate()).forEach(addConsumer);

    dst.stream().filter(((Predicate<T>) src::contains).negate()).forEach(removeConsumer);
   ...
   *
   * @return A list containing the elements of source that are not in filter, using <code>
   *     equivalence</code> to distinguish different elements.
   */
  public static <T> List<T> difference(
      Iterable<? extends T> source,
      Iterable<? extends T> filter,
   ...
   * <strong>WARNING:</strong> very low efficiency (O(N*N))
   *
   * @return A list containing the elements of source that are also in filter, using <code>
   *     equivalence</code> to distinguish different elements.
   */
