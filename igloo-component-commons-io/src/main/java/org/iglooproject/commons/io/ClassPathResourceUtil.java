...
/**
 * Some utilities to handle classpath:/ urls without Spring. Prefix may be <em>classpath:/</em> or
 * <em>classpath:</em>.
 */
public class ClassPathResourceUtil {
  ...
  /**
   * This default implementation uses {@link ClassPathResourceUtil} {@link ClassLoader} for resource
   * lookup.
   */
  ...
  /**
   * Use a custom {@link ClassLoader} for resource lookup. If you don't know if you need it, just
   * use no-arg {@link ClassPathResourceUtil#ClassPathResourceUtil()} constructor.
   */
  ...
  /**
   * This package-protected constructor allow to provide a custom-implementation {@link
   * ClassPathResourceHelper} (internal API).
   */
  ClassPathResourceUtil(ClassLoader classLoader, ClassPathResourceHelper helper) {
    super();
    this.classLoader =
        Optional.ofNullable(classLoader).orElse(ClassPathResourceUtil.class.getClassLoader());
    this.helper = Optional.ofNullable(helper).orElse(new ClassPathResourceHelper());
  }

  /**
   * Extract last modified date (as epoch) from a <em>classpath:/</em> url. For jar resources,
   * returned date is computed from jar file.
   *
  ...
  /**
   * Extract text content from a resource. Content must be an text encoded with the provided
   * encoding.
   *
   * @see ClassPathResourceUtil
   * @param classpathUrl <em>classpath:...</em> resource url
  ...
  /**
   * Provide an {@link InputStream} for the provided <em>classpath:...</em> url. Closing the
   * provided {@link InputStream} after usage is needed.
   *
   * @see ClassPathResourceUtil
   * @param classpathUrl <em>classpath:...</em> resource url
  ...
  private long lastModified(String classPathUrl, URL url) throws IOException {
    if (protocol(url).equals("jar")) {
      ...
      throw new IllegalArgumentException(
          String.format("protocol for %s not supported (original url: %s)", url, classPathUrl));
    }
  }
