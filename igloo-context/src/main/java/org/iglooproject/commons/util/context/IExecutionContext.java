 ...
 *
 * <p>Setting up a context typically involves altering a state, and tearing it down typically means
 * restoring the previous state (or any clean state, depending on the implementor's constraints)).
 *
 * <p>The state may for example be stored in a {@link ThreadLocal}, or in the object that created
 * this {@link IExecutionContext}.
 *
...
public interface IExecutionContext {
  ...
  /**
   * Set up the context for execution and return an {@link ITearDownHandle} that will tear down the
   * context upon {@link ITearDownHandle#close()}.
   *
  ...
  /**
   * Set up the context, run the given {@link Callable}, tear down the context, and return the
   * callable's result (if any, i.e. if it didn't throw an exception).
   *
   * <p>The implementation must tear down the context even if the runnable throws an exception.
   */
